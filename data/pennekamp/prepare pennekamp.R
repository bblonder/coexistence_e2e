...
data_wide = data %>%
  ...
  pivot_wider(names_from='predicted_species', values_from='species_biomass',names_glue = "{predicted_species}.star")
...
# replace all NAs with 0s
  replace(is.na(.), 0) %>%
  mutate(stable=NA,feasible=1,richness=NA) %>%
# rearrange
  select(all_of(names_all), stable, feasible, richness, all_of(paste(names_all,"star",sep=".")),temperature)

data_wide_processed$richness = rowSums((data_wide_processed %>% select(contains('star'))) > 0)
