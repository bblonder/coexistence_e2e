...
# do letter indexing to be consistent with other datasets
names(species_all) = c(letters,paste(letters, letters,sep=""))[1:length(species_all)]

df_empty = data.frame(matrix(data=0, nrow=1, ncol=length(species_all)+3+length(species_all) + 2))
colnames(df_empty) = c(species_all,"stable","feasible","richness",paste(species_all,"star",sep="."), 'food','temperature')
...
for (i in 1:nrow(pairwise_long_all))
{
  ...
  df_this = df_empty
  df_this[1,sp1_this] = 0.5 # relative abundance
  df_this[1,sp2_this] = 0.5 # relative abundance 
  
  df_this[1,"stable"] = NA
  df_this[1,"feasible"] = 1
  df_this[1,"richness"] = ifelse(outcome_this %in% c("1","0"), 1, 2) # assuming there are no extinctions of everyone, we can check
  ...
  df_this[1,"food"] = food_this
  df_this[1,"temperature"] = temperature_this
...
for (i in 1:nrow(multispecies))
{
  ...
  {
    df_this[1, name_this] = multispecies[i,name_this] / 100 # scale as relative abundnace
    df_this[1, paste(name_this,"star",sep=".")] = multispecies[i,paste(name_this,"outcome",sep=".")] / 100 # this scales to relative abundance
  }
  
  df_this[1,"food"] = multispecies[i,"food"]
  df_this[1,"temperature"] = multispecies[i,"temperature"]
...
names(result_all)[match(species_all, names(result_all))] = names(species_all)
names(result_all)[match(paste(species_all,"star",sep="."), names(result_all))] = paste(names(species_all),"star",sep=".")

write.csv(result_all, file='data_gilpin.csv', row.names=FALSE)
write.csv(data.frame(letter=names(species_all),abbreviation=species_all), file='names_gilpin.csv', row.names=FALSE)
